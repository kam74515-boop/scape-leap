...
export class ThemeStore implements IThemeStore {
   ...
   * @param collapsed 传入布尔则强制设置；省略则在当前态上翻转（undefined 视为展开）
   */
  toggleSidebar = (collapsed?: boolean) => {
    if (collapsed === undefined) {
      // undefined 当展开：避免 !undefined===true 把首点误当成「已收起」
      this.sidebarCollapsed = !(this.sidebarCollapsed === true);
    } else {
      this.sidebarCollapsed = collapsed;
    }
    localStorage.setItem("app_sidebar_collapsed", String(!!this.sidebarCollapsed));
  };
