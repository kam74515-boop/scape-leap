...
export class BaseWorkspaceRootStore implements IWorkspaceRootStore {
  ...
  fetchWorkspaces = async () => {
    ...
    try {
      ...
      const now = new Date();
      const demoWorkspace: IWorkspace = {
        id: "ws-demo",
        name: "构境工作室",
        slug: "formscape",
        url: "/formscape",
        logo_url: null,
        total_members: 1,
        created_by: "user-local-1",
        updated_by: "user-local-1",
      ...
      runInAction(() => {
        set(this.workspaces, [demoWorkspace.id], demoWorkspace);
      });
