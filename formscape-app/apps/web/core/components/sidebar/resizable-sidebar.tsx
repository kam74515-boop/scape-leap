...
export function ResizableSidebar({
  ...
  // 仅在真正布尔态变化时回写，避免 isCollapsed 为 undefined 时触发 toggle 翻转
  const collapsedBool = !!isCollapsed;
  const prevCollapsedRef = useRef<boolean | null>(null);
  useEffect(() => {
    if (prevCollapsedRef.current === collapsedBool) return;
    prevCollapsedRef.current = collapsedBool;
    onCollapsedChange?.(collapsedBool);
  }, [collapsedBool, onCollapsedChange]);
  ...
  return (
    <>
      ...
      >
        <aside
          className={cn(
            // 去掉 pt-3：上下由 SidebarWrapper 对称控制，避免 L2 上大下小
            "group/sidebar relative flex h-full min-h-0 w-full flex-col overflow-hidden bg-surface-1",
      ...
      >
        <aside
          className={cn(
            "group/sidebar relative z-20 flex h-full min-h-0 w-full flex-col overflow-hidden bg-surface-1",
