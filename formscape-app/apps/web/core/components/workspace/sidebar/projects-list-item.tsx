...
import { Settings, Share2, LogOut, MoreHorizontal } from "@/icons";
// plane imports
...
export const SidebarProjectsListItem = observer(function SidebarProjectsListItem(props: Props) {
  ...
  useEffect(() => {
    ...
    if (URLProjectId === project?.id && projectRef.current) {
  ...
  // 构境：L2 只展示项目行，点击进入默认任务页；子导航仅顶栏 Tab
              ...
              <CustomMenu
                customButton={
                  <span
                    ...
                    className="grid size-6 place-items-center rounded-md text-placeholder hover:bg-layer-transparent-hover hover:text-secondary"
                  >
